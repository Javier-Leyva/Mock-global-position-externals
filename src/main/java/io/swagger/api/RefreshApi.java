...
@RequestMapping(value = "/global_position")
public interface RefreshApi {

    @ApiOperation(value = "Performs manual global position bank update and provides information about contracts and balances of contracts of a customer.", nickname = "refreshPost", notes = "This operation performs a manual update of the global position information of one particular user bank. This operation allows us to obtain total balances by product families and contract balances. You can group balances by product family. It allows filters by family of products, by contractual situation (active, canceled). - if familyProduct is informed, the return will be only for this family - If active_only is true only active contracts take it, with the false return there will be active and canceled contracts.", response = GetGlobalPositionResponse.class, authorizations = {
        @Authorization(value = "JWT Profile", scopes = {
            @AuthorizationScope(scope = "gblpos.read", description = "Global position read scope")
            }),
        @Authorization(value = "x-IBM-Client-Id")
    }, tags={ "global-position", })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "OK", response = GetGlobalPositionResponse.class),
